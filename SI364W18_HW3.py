...
app.config["SQLALCHEMY_DATABASE_URI"] = "postgresql://localhost/sabomjHW3"
## Provided:
...
def see_all_tweets():
    ...
    tweetdb = Tweet.query.all()
    tweets_python = []
    for x in tweetdb:
        user = User.query.filter_by(id=x.user_id).first()
        tweets_python.append((x.text, user.username))
    print(tweets_python)
    return render_template('all_tweets.html', tweets_html=tweets_python)
...
def get_longest_tweet():
    ...
    tweetdb = Tweet.query.all()
    for x in tweetdb:
        if len(x.text.replace(' ','')) > longest:
            longest_tweet = x.text
            longest = len(longest_tweet)

    user = User.query.filter_by(id = x.user_id).first()
    username = user.username
    name = user.display_name
    return render_template('longest_tweet.html', longest_tweet = longest_tweet, user = username, display_name = name)
#
